...
2018/12/19  
			設定変更かつ保存していない場合、終了前に確認メッセージ  
			Datagridviewの変更確認はCellValueChanged?  プロファイル変更時に確認＆初期化
			エラーログを残す。  ex.message & ex.StackTrace  
			仮想カメラ映像の切り出し方法を変更（videoio_... →capturecv.read(frame)  
			Update表示を消去。バージョン確認を任意に行う（起動時に確認のチェックを付けたほうがいい？）  
